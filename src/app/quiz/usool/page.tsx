    ...
    <div className="min-h-screen bg-andalusian animate-pattern flex flex-col font-sans text-slate-900 selection:bg-emerald-100 selection:text-emerald-900">
      ...
      <nav className="bg-white/90 backdrop-blur-xl border-b border-emerald-100/50 sticky top-0 z-50">
        <div className="max-w-3xl mx-auto px-6 h-16 flex items-center justify-between">
          <Link href="/quiz" className="flex items-center gap-2 text-slate-500 hover:text-emerald-800 transition">
            <ArrowLeft size={20} />
        ...
        <div className="w-full max-w-3xl">
          ...
          {showScore ? (
            ...
            <div className="bg-white border border-emerald-100 rounded-3xl p-12 text-center shadow-xl shadow-emerald-900/5 animate-in zoom-in-95 duration-500">
              ...
              <p className="text-slate-500 mb-6">You scored {score} out of {quizData.questions.length}</p>
              
              {/* Cloud Save Indicator */}
              <div className="mb-8 flex justify-center">
                 {isSaving ? (
                   <span className="text-xs font-bold text-emerald-600 flex items-center gap-2 bg-emerald-50 px-3 py-1 rounded-full"><Loader2 size={12} className="animate-spin"/> Saving to Profile...</span>
                 ) : (
                   <span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full">{saveMessage}</span>
                 )}
              </div>
              ...
              <div className="flex justify-center gap-4">
                ...
                >
                  <BookOpen size={18} /> View Certificate
                </Link>
              </div>
            </div>
          ) : (
